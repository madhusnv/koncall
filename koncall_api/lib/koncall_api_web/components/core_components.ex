defmodule KoncallApiWeb.CoreComponents do
  @moduledoc """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(KoncallApiWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(KoncallApiWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  JS commands may be used by the `:on_cancel` to configure
  ...
  def modal(assigns) do
    ...
    >
      <div id={"#{@id}-bg"} class="fixed inset-0 bg-[#2d1810]/60 backdrop-blur-sm transition-opacity" aria-hidden="true" />
      <div
      ...
      >
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
          <div
            id={"#{@id}-panel"}
            class="w-full max-w-3xl overflow-hidden rounded-2xl p-8 text-left align-middle shadow-2xl ring-2 transition-all transform scale-95 opacity-0"
            style="background: var(--color-bg-primary); border: 2px solid var(--color-border-light); box-shadow: 0 25px 50px -12px rgba(45, 24, 16, 0.25);"
          >
            <div class="absolute right-4 top-4">
              <button
                phx-click={JS.exec(@on_cancel, "phx-remove")}
                type="button"
                class="flex items-center justify-center w-10 h-10 rounded-xl transition-all hover:scale-110"
