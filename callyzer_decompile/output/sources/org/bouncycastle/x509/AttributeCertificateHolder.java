...
public class AttributeCertificateHolder implements CertSelector, Selector {
    ...
    public AttributeCertificateHolder(int i10, String str, String str2, byte[] bArr) {
        this.holder = new Holder(new ObjectDigestInfo(i10, new ASN1ObjectIdentifier(str2), new AlgorithmIdentifier(new ASN1ObjectIdentifier(str)), Arrays.clone(bArr)));
    }
    ...
    private Object[] getNames(GeneralName[] generalNameArr) {
        ...
        for (int i10 = 0; i10 != generalNameArr.length; i10++) {
            if (generalNameArr[i10].getTagNo() == 4) {
                try {
                    arrayList.add(new X500Principal(generalNameArr[i10].getName().toASN1Primitive().getEncoded()));
