...
public class SP800SecureRandomBuilder {
    ...
    public static String getSimplifiedName(Digest digest) {
        ...
        if (iIndexOf <= 0 || algorithmName.startsWith("SHA3")) {
            return algorithmName;
        }
        return algorithmName.substring(0, iIndexOf) + algorithmName.substring(iIndexOf + 1);
    }

    public SP800SecureRandom buildCTR(BlockCipher blockCipher, int i10, byte[] bArr, boolean z6) {
        return new SP800SecureRandom(this.random, this.entropySourceProvider.get(this.entropyBitsRequired), new CTRDRBGProvider(blockCipher, i10, bArr, this.personalizationString, this.securityStrength), z6);
    }

    public SP800SecureRandom buildHMAC(Mac mac, byte[] bArr, boolean z6) {
        return new SP800SecureRandom(this.random, this.entropySourceProvider.get(this.entropyBitsRequired), new HMacDRBGProvider(mac, bArr, this.personalizationString, this.securityStrength), z6);
    }

    public SP800SecureRandom buildHash(Digest digest, byte[] bArr, boolean z6) {
        return new SP800SecureRandom(this.random, this.entropySourceProvider.get(this.entropyBitsRequired), new HashDRBGProvider(digest, bArr, this.personalizationString, this.securityStrength), z6);
    }
