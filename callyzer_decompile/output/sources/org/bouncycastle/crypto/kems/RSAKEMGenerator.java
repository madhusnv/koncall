...
public class RSAKEMGenerator implements EncapsulatedSecretGenerator {
    ...
    public RSAKEMGenerator(int i10, DerivationFunction derivationFunction, SecureRandom secureRandom) {
        this.keyLen = i10;
        this.kdf = derivationFunction;
        this.rnd = secureRandom;
    }

    public static byte[] generateKey(DerivationFunction derivationFunction, BigInteger bigInteger, BigInteger bigInteger2, int i10) {
        derivationFunction.init(new KDFParameters(BigIntegers.asUnsignedByteArray((bigInteger.bitLength() + 7) / 8, bigInteger2), null));
        byte[] bArr = new byte[i10];
        derivationFunction.generateBytes(bArr, 0, i10);
        return bArr;
    }
