...
public class DHAgreement {
    ...
    public BigInteger calculateAgreement(DHPublicKeyParameters dHPublicKeyParameters, BigInteger bigInteger) {
        ...
        BigInteger p4 = this.dhParams.getP();
        BigInteger y10 = dHPublicKeyParameters.getY();
        if (y10 != null) {
            BigInteger bigInteger2 = ONE;
            if (y10.compareTo(bigInteger2) > 0 && y10.compareTo(p4.subtract(bigInteger2)) < 0) {
                BigInteger bigIntegerModPow = y10.modPow(this.privateValue, p4);
                if (bigIntegerModPow.equals(bigInteger2)) {
                    throw new IllegalStateException("Shared key can't be 1");
                }
                return bigInteger.modPow(this.key.getX(), p4).multiply(bigIntegerModPow).mod(p4);
            }
        }
