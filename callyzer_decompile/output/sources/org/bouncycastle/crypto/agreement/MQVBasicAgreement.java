...
public class MQVBasicAgreement implements BasicAgreement {
    ...
    private BigInteger calculateDHMQVAgreement(DHParameters dHParameters, DHPrivateKeyParameters dHPrivateKeyParameters, DHPublicKeyParameters dHPublicKeyParameters, DHPrivateKeyParameters dHPrivateKeyParameters2, DHPublicKeyParameters dHPublicKeyParameters2, DHPublicKeyParameters dHPublicKeyParameters3) {
        ...
        BigInteger bigIntegerPow = BigInteger.valueOf(2L).pow((q10.bitLength() + 1) / 2);
        return dHPublicKeyParameters3.getY().multiply(dHPublicKeyParameters.getY().modPow(dHPublicKeyParameters3.getY().mod(bigIntegerPow).add(bigIntegerPow), dHParameters.getP())).modPow(dHPrivateKeyParameters2.getX().add(dHPublicKeyParameters2.getY().mod(bigIntegerPow).add(bigIntegerPow).multiply(dHPrivateKeyParameters.getX())).mod(q10), dHParameters.getP());
    }
