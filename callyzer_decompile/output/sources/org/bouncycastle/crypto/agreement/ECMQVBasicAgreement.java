...
public class ECMQVBasicAgreement implements BasicAgreement {
    ...
    private ECPoint calculateMqvAgreement(ECDomainParameters eCDomainParameters, ECPrivateKeyParameters eCPrivateKeyParameters, ECPrivateKeyParameters eCPrivateKeyParameters2, ECPublicKeyParameters eCPublicKeyParameters, ECPublicKeyParameters eCPublicKeyParameters2, ECPublicKeyParameters eCPublicKeyParameters3) {
        BigInteger n6 = eCDomainParameters.getN();
        int iBitLength = (n6.bitLength() + 1) / 2;
        ...
        BigInteger bigIntegerMod = eCPrivateKeyParameters.getD().multiply(eCPointCleanPoint.getAffineXCoord().toBigInteger().mod(bigIntegerShiftLeft).setBit(iBitLength)).add(eCPrivateKeyParameters2.getD()).mod(n6);
        BigInteger bit = eCPointCleanPoint3.getAffineXCoord().toBigInteger().mod(bigIntegerShiftLeft).setBit(iBitLength);
        BigInteger bigIntegerMod2 = eCDomainParameters.getH().multiply(bigIntegerMod).mod(n6);
        return ECAlgorithms.sumOfTwoMultiplies(eCPointCleanPoint2, bit.multiply(bigIntegerMod2).mod(n6), eCPointCleanPoint3, bigIntegerMod2);
    }
