...
public class JPAKEUtil {
    ...
    public static BigInteger calculateMacTag(String str, String str2, BigInteger bigInteger, BigInteger bigInteger2, BigInteger bigInteger3, BigInteger bigInteger4, BigInteger bigInteger5, Digest digest) {
        byte[] bArrCalculateMacKey = calculateMacKey(bigInteger5, digest);
    ...
    public static BigInteger[] calculateZeroKnowledgeProof(BigInteger bigInteger, BigInteger bigInteger2, BigInteger bigInteger3, BigInteger bigInteger4, BigInteger bigInteger5, String str, Digest digest, SecureRandom secureRandom) {
        BigInteger bigIntegerCreateRandomInRange = BigIntegers.createRandomInRange(ZERO, bigInteger2.subtract(ONE), secureRandom);
        BigInteger bigIntegerModPow = bigInteger3.modPow(bigIntegerCreateRandomInRange, bigInteger);
        return new BigInteger[]{bigIntegerModPow, bigIntegerCreateRandomInRange.subtract(bigInteger5.multiply(calculateHashForZeroKnowledgeProof(bigInteger3, bigIntegerModPow, bigInteger4, str, digest))).mod(bigInteger2)};
    }
    ...
    public static void validateNotNull(Object obj, String str) {
        if (obj == null) {
            throw new NullPointerException(AbstractC1452a.m4561h(str, " must not be null"));
        }
    }

    public static void validateParticipantIdsDiffer(String str, String str2) throws CryptoException {
        if (str.equals(str2)) {
            throw new CryptoException(AbstractC5601a.m11238i("Both participants are using the same participantId (", str, "). This is not allowed. Each participant must use a unique participantId."));
        }
    }

    public static void validateParticipantIdsEqual(String str, String str2) throws CryptoException {
        if (!str.equals(str2)) {
            throw new CryptoException(a1.m14333m("Received payload from incorrect partner (", str2, "). Expected to receive payload from ", str, "."));
        }
    }
    ...
    public static void validateZeroKnowledgeProof(BigInteger bigInteger, BigInteger bigInteger2, BigInteger bigInteger3, BigInteger bigInteger4, BigInteger[] bigIntegerArr, String str, Digest digest) throws CryptoException {
        ...
        BigInteger bigIntegerCalculateHashForZeroKnowledgeProof = calculateHashForZeroKnowledgeProof(bigInteger3, bigInteger5, bigInteger4, str, digest);
        if (bigInteger4.compareTo(ZERO) != 1 || bigInteger4.compareTo(bigInteger) != -1 || bigInteger4.modPow(bigInteger2, bigInteger).compareTo(ONE) != 0 || bigInteger3.modPow(bigInteger6, bigInteger).multiply(bigInteger4.modPow(bigIntegerCalculateHashForZeroKnowledgeProof, bigInteger)).mod(bigInteger).compareTo(bigInteger5) != 0) {
            throw new CryptoException("Zero-knowledge proof validation failed");
        }
    }

    public static BigInteger calculateS(BigInteger bigInteger, char[] cArr) {
    ...
    private static void updateDigestIncludingSize(Digest digest, BigInteger bigInteger) {
        byte[] bArrAsUnsignedByteArray = BigIntegers.asUnsignedByteArray(bigInteger);
        digest.update(intToByteArray(bArrAsUnsignedByteArray.length), 0, 4);
        digest.update(bArrAsUnsignedByteArray, 0, bArrAsUnsignedByteArray.length);
        Arrays.fill(bArrAsUnsignedByteArray, (byte) 0);
    }
    ...
    private static void updateMac(Mac mac, BigInteger bigInteger) {
        byte[] bArrAsUnsignedByteArray = BigIntegers.asUnsignedByteArray(bigInteger);
        mac.update(bArrAsUnsignedByteArray, 0, bArrAsUnsignedByteArray.length);
        Arrays.fill(bArrAsUnsignedByteArray, (byte) 0);
    }

    public static BigInteger calculateS(char[] cArr) {
        return new BigInteger(1, Strings.toUTF8ByteArray(cArr));
    }
}
