...
public class HMacDSAKCalculator implements DSAKCalculator {
    ...
    /* renamed from: K */
    private final byte[] f27791K;

    /* renamed from: V */
    private final byte[] f27792V;
    private final HMac hMac;

    /* renamed from: n */
    private BigInteger f27793n;

    public HMacDSAKCalculator(Digest digest) {
        HMac hMac = new HMac(digest);
        this.hMac = hMac;
        this.f27792V = new byte[hMac.getMacSize()];
        this.f27791K = new byte[hMac.getMacSize()];
    }

    private BigInteger bitsToInt(byte[] bArr) {
        BigInteger bigInteger = new BigInteger(1, bArr);
        return bArr.length * 8 > this.f27793n.bitLength() ? bigInteger.shiftRight((bArr.length * 8) - this.f27793n.bitLength()) : bigInteger;
    }

    @Override // org.bouncycastle.crypto.signers.DSAKCalculator
    public void init(BigInteger bigInteger, BigInteger bigInteger2, byte[] bArr) {
        this.f27793n = bigInteger;
        Arrays.fill(this.f27792V, (byte) 1);
        Arrays.fill(this.f27791K, (byte) 0);
        ...
        this.hMac.init(new KeyParameter(this.f27791K));
        HMac hMac = this.hMac;
        byte[] bArr4 = this.f27792V;
        ...
        this.hMac.doFinal(this.f27791K, 0);
        this.hMac.init(new KeyParameter(this.f27791K));
        HMac hMac2 = this.hMac;
        byte[] bArr5 = this.f27792V;
        hMac2.update(bArr5, 0, bArr5.length);
        this.hMac.doFinal(this.f27792V, 0);
        HMac hMac3 = this.hMac;
        byte[] bArr6 = this.f27792V;
    ...
    public BigInteger nextK() {
        int unsignedByteLength = BigIntegers.getUnsignedByteLength(this.f27793n);
        byte[] bArr = new byte[unsignedByteLength];
        while (true) {
            int i10 = 0;
            while (i10 < unsignedByteLength) {
                HMac hMac = this.hMac;
                byte[] bArr2 = this.f27792V;
                hMac.update(bArr2, 0, bArr2.length);
                this.hMac.doFinal(this.f27792V, 0);
                int iMin = Math.min(unsignedByteLength - i10, this.f27792V.length);
                System.arraycopy(this.f27792V, 0, bArr, i10, iMin);
                i10 += iMin;
            }
            BigInteger bigIntegerBitsToInt = bitsToInt(bArr);
            if (bigIntegerBitsToInt.compareTo(ZERO) > 0 && bigIntegerBitsToInt.compareTo(this.f27793n) < 0) {
                return bigIntegerBitsToInt;
            }
            HMac hMac2 = this.hMac;
            byte[] bArr3 = this.f27792V;
