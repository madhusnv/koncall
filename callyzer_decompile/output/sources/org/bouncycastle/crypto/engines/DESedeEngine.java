...
public class DESedeEngine extends DESBase implements BlockCipher {
    ...
    public void init(boolean z6, CipherParameters cipherParameters) {
        if (!(cipherParameters instanceof KeyParameter)) {
            throw new IllegalArgumentException(AbstractC4801l.m9736j("invalid parameter passed to DESede init - ", cipherParameters));
        }
        ...
        this.workingKey1 = generateWorkingKey(z6, bArr);
        byte[] bArr2 = new byte[8];
        System.arraycopy(key, 8, bArr2, 0, 8);
        this.workingKey2 = generateWorkingKey(!z6, bArr2);
        if (key.length == 24) {
            byte[] bArr3 = new byte[8];
            System.arraycopy(key, 16, bArr3, 0, 8);
            this.workingKey3 = generateWorkingKey(z6, bArr3);
        } else {
    ...
    public int processBlock(byte[] bArr, int i10, byte[] bArr2, int i11) {
        ...
        if (this.forEncryption) {
            desFunc(iArr, bArr, i10, bArr3, 0);
            desFunc(this.workingKey2, bArr3, 0, bArr3, 0);
            desFunc(this.workingKey3, bArr3, 0, bArr2, i11);
        } else {
            desFunc(this.workingKey3, bArr, i10, bArr3, 0);
            desFunc(this.workingKey2, bArr3, 0, bArr3, 0);
            desFunc(this.workingKey1, bArr3, 0, bArr2, i11);
        }
        return 8;
    }
