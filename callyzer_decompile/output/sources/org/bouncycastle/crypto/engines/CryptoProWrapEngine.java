...
public class CryptoProWrapEngine extends GOST28147WrapEngine {
    ...
    private static byte[] cryptoProDiversify(byte[] bArr, byte[] bArr2, byte[] bArr3) {
        for (int i10 = 0; i10 != 8; i10++) {
            ...
            for (int i13 = 0; i13 != 8; i13++) {
                int iLittleEndianToInt = Pack.littleEndianToInt(bArr, i13 * 4);
                if (bitSet(bArr2[i10], i13)) {
                    i11 += iLittleEndianToInt;
                } else {
    ...
    public void init(boolean z6, CipherParameters cipherParameters) {
        ...
        super.init(z6, sBox != null ? new ParametersWithUKM(new ParametersWithSBox(keyParameter2, sBox), parametersWithUKM.getUKM()) : new ParametersWithUKM(keyParameter2, parametersWithUKM.getUKM()));
    }
}
