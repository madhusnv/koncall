...
public class IESCipher extends BaseCipherSpi {
    ...
    public static class XIESwithCipher extends IESCipher {
        public XIESwithCipher(BlockCipher blockCipher, int i10) {
            this(blockCipher, i10, DigestFactory.createSHA1(), DigestFactory.createSHA1());
        }

        public XIESwithCipher(BlockCipher blockCipher, int i10, Digest digest, Digest digest2) {
            super(new IESEngine(new XDHBasicAgreement(), new KDF2BytesGenerator(digest), new HMac(digest2), new PaddedBufferedBlockCipher(blockCipher)), i10);
        }
    }
    ...
    public int engineGetKeySize(Key key) {
        ...
        throw new IllegalArgumentException(m0.m7378k("unknown XDH key algorithm ", algorithm));
    }
    ...
    public byte[] engineDoFinal(byte[] bArr, int i10, int i11) throws BadBlockException {
        ...
        final boolean z6 = (asymmetricKeyParameter2 instanceof X25519PublicKeyParameters) || (asymmetricKeyParameter2 instanceof X25519PrivateKeyParameters);
        int i13 = z6 ? 256 : 448;
        int i14 = this.state;
        if (i14 == 1 || i14 == 3) {
            AsymmetricCipherKeyPairGenerator x25519KeyPairGenerator = z6 ? new X25519KeyPairGenerator() : new X448KeyPairGenerator();
            x25519KeyPairGenerator.init(new KeyGenerationParameters(this.random, i13));
            try {
                this.engine.init(this.key, iESWithCipherParameters, new EphemeralKeyPairGenerator(x25519KeyPairGenerator, new KeyEncoder() { // from class: org.bouncycastle.jcajce.provider.asymmetric.edec.IESCipher.1
                    ...
                    public byte[] getEncoded(AsymmetricKeyParameter asymmetricKeyParameter3) {
                        return z6 ? ((X25519PublicKeyParameters) asymmetricKeyParameter3).getEncoded() : ((X448PublicKeyParameters) asymmetricKeyParameter3).getEncoded();
                    }
                }));
                return this.engine.processBlock(byteArray, 0, byteArray.length);
            } catch (Exception e10) {
        ...
        try {
            this.engine.init(asymmetricKeyParameter2, iESWithCipherParameters, new XIESPublicKeyParser(z6));
            return this.engine.processBlock(byteArray, 0, byteArray.length);
