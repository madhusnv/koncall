...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, CertificateException, InvalidAlgorithmParameterException {
        if (!isAlgIdEqual(this.f27815c.getSignatureAlgorithm(), this.f27815c.getTBSCertificate().getSignature())) {
        ...
        try {
            ...
            this.f27815c.getTBSCertificate().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws NoSuchAlgorithmException, SignatureException, IOException, InvalidKeyException, CertificateException, InvalidAlgorithmParameterException {
        boolean z6 = publicKey instanceof CompositePublicKey;
        int i10 = 0;
        if (z6 && X509SignatureUtil.isCompositeAlgorithm(this.f27815c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence aSN1Sequence = ASN1Sequence.getInstance(this.f27815c.getSignatureAlgorithm().getParameters());
            ASN1Sequence aSN1Sequence2 = ASN1Sequence.getInstance(ASN1BitString.getInstance(this.f27815c.getSignature()).getBytes());
            boolean z10 = false;
        ...
        if (!X509SignatureUtil.isCompositeAlgorithm(this.f27815c.getSignatureAlgorithm())) {
            Signature signatureCreateSignature = signatureCreator.createSignature(X509SignatureUtil.getSignatureName(this.f27815c.getSignatureAlgorithm()));
            if (!z6) {
                checkSignature(publicKey, signatureCreateSignature, this.f27815c.getSignatureAlgorithm().getParameters(), getSignature());
                return;
            }
            List<PublicKey> publicKeys2 = ((CompositePublicKey) publicKey).getPublicKeys();
            while (i10 != publicKeys2.size()) {
                try {
                    checkSignature(publicKeys2.get(i10), signatureCreateSignature, this.f27815c.getSignatureAlgorithm().getParameters(), getSignature());
                    return;
        ...
        ASN1Sequence aSN1Sequence3 = ASN1Sequence.getInstance(this.f27815c.getSignatureAlgorithm().getParameters());
        ASN1Sequence aSN1Sequence4 = ASN1Sequence.getInstance(ASN1BitString.getInstance(this.f27815c.getSignature()).getBytes());
        boolean z11 = false;
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f27815c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f27815c, "2.5.29.37");
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public Collection getIssuerAlternativeNames() {
        return getAlternativeNames(this.f27815c, Extension.issuerAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getIssuerDN() {
        return new X509Principal(this.f27815c.getIssuer());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getIssuerUniqueID() {
        ASN1BitString issuerUniqueId = this.f27815c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f27815c.getIssuer();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f27815c.getIssuer().getEncoded(ASN1Encoding.DER));
    ...
    public Date getNotAfter() {
        return this.f27815c.getEndDate().getDate();
    }

    @Override // java.security.cert.X509Certificate
    public Date getNotBefore() {
        return this.f27815c.getStartDate().getDate();
    }

    @Override // java.security.cert.Certificate
    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f27815c.getSubjectPublicKeyInfo());
        } catch (IOException e2) {
            throw Exceptions.illegalStateException("failed to recover public key: " + e2.getMessage(), e2);
        }
    }

    @Override // java.security.cert.X509Certificate
    public BigInteger getSerialNumber() {
        return this.f27815c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f27815c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f27815c.getSignature().getOctets();
    }

    @Override // java.security.cert.X509Certificate
    public Collection getSubjectAlternativeNames() {
        return getAlternativeNames(this.f27815c, Extension.subjectAlternativeName.getId());
    }

    @Override // java.security.cert.X509Certificate
    public Principal getSubjectDN() {
        return new X509Principal(this.f27815c.getSubject());
    }

    @Override // java.security.cert.X509Certificate
    public boolean[] getSubjectUniqueID() {
        ASN1BitString subjectUniqueId = this.f27815c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f27815c.getSubject();
    }

    @Override // java.security.cert.X509Certificate
    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f27815c.getSubject().getEncoded(ASN1Encoding.DER));
        } catch (IOException unused) {
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f27815c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e2) {
            throw new CertificateEncodingException(e2.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f27815c.getTBSCertificate();
    }

    @Override // java.security.cert.X509Certificate
    public int getVersion() {
        return this.f27815c.getVersionNumber();
    }

    @Override // java.security.cert.X509Extension
    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f27815c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration enumerationOids = extensions.oids();
        while (enumerationOids.hasMoreElements()) {
    ...
    public void checkValidity(Date date) throws CertificateNotYetValidException, CertificateExpiredException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f27815c.getEndDate().getTime());
        }
        if (date.getTime() >= getNotBefore().getTime()) {
            return;
        }
        throw new CertificateNotYetValidException("certificate not valid till " + this.f27815c.getStartDate().getTime());
    }

    @Override // java.security.cert.X509Extension
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f27815c, str);
        if (extensionValue == null) {
            return null;
        }
        try {
            ...
            throw Exceptions.illegalStateException("error parsing " + e2.getMessage(), e2);
        }
    }
