...
public class EC5Util {
    ...
    public static class CustomCurves {
        ...
        public static ECCurve substitute(ECCurve eCCurve) {
    ...
    public static ECParameterSpec convertToSpec(X962Parameters x962Parameters, ECCurve eCCurve) {
        ...
        if (aSN1Sequence.size() > 3) {
            ...
            return x9ECParameters.getH() != null ? new ECParameterSpec(ellipticCurveConvertCurve, convertPoint(x9ECParameters.getG()), x9ECParameters.getN(), x9ECParameters.getH().intValue()) : new ECParameterSpec(ellipticCurveConvertCurve, convertPoint(x9ECParameters.getG()), x9ECParameters.getN(), 1);
        }
        ...
        return new ECNamedCurveSpec(ECGOST3410NamedCurves.getName(gOST3410PublicKeyAlgParameters.getPublicKeyParamSet()), convertCurve(parameterSpec.getCurve(), parameterSpec.getSeed()), convertPoint(parameterSpec.getG()), parameterSpec.getN(), parameterSpec.getH());
    }
    ...
    public static ECCurve getCurve(ProviderConfiguration providerConfiguration, X962Parameters x962Parameters) {
        ...
        if (!x962Parameters.isNamedCurve()) {
            ...
            if (acceptableNamedCurves.isEmpty()) {
                return aSN1Sequence.size() > 3 ? X9ECParameters.getInstance(aSN1Sequence).getCurve() : ECGOST3410NamedCurves.getByOIDX9(ASN1ObjectIdentifier.getInstance(aSN1Sequence.getObjectAt(0))).getCurve();
            }
    ...
    public static ECCurve convertCurve(EllipticCurve ellipticCurve) {
        ...
        if (field instanceof ECFieldFp) {
            return CustomCurves.substitute(new ECCurve.Fp(((ECFieldFp) field).getP(), a10, b10, null, null));
        }
        ECFieldF2m eCFieldF2m = (ECFieldF2m) field;
        int m10 = eCFieldF2m.getM();
        int[] iArrConvertMidTerms = ECUtil.convertMidTerms(eCFieldF2m.getMidTermsOfReductionPolynomial());
        return new ECCurve.F2m(m10, iArrConvertMidTerms[0], iArrConvertMidTerms[1], iArrConvertMidTerms[2], a10, b10, (BigInteger) null, (BigInteger) null);
    }
