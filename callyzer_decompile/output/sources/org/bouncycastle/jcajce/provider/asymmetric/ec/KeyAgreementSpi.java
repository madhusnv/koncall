...
public class KeyAgreementSpi extends BaseAgreementSpi {
    ...
    public void doInitFromKey(Key key, AlgorithmParameterSpec algorithmParameterSpec, SecureRandom secureRandom) throws InvalidKeyException, InvalidAlgorithmParameterException {
        ...
        if (obj instanceof ECMQVBasicAgreement) {
            ...
            if (!z6 && !(algorithmParameterSpec instanceof MQVParameterSpec)) {
                StringBuilder sb2 = new StringBuilder();
                sb2.append(this.kaAlgorithm);
                sb2.append(" key agreement requires ");
                throw new InvalidAlgorithmParameterException(AbstractC1452a.m4564k(sb2, getSimpleName(MQVParameterSpec.class), " for initialisation"));
            }
            if (z6) {
        ...
        if (!(algorithmParameterSpec instanceof DHUParameterSpec)) {
            if (!(key instanceof PrivateKey)) {
                ...
                throw new InvalidKeyException(AbstractC1452a.m4564k(sb3, getSimpleName(ECPrivateKey.class), " for initialisation"));
            }
    ...
    public Key engineDoPhase(Key key, boolean z6) throws InvalidKeyException {
        ...
        if (obj instanceof ECMQVBasicAgreement) {
            if (key instanceof MQVPublicKey) {
                ...
                cipherParametersGeneratePublicKeyParameter = new MQVPublicParameters((ECPublicKeyParameters) ECUtils.generatePublicKeyParameter(mQVPublicKey.getStaticKey()), (ECPublicKeyParameters) ECUtils.generatePublicKeyParameter(mQVPublicKey.getEphemeralKey()));
            } else {
                cipherParametersGeneratePublicKeyParameter = new MQVPublicParameters((ECPublicKeyParameters) ECUtils.generatePublicKeyParameter((PublicKey) key), (ECPublicKeyParameters) ECUtils.generatePublicKeyParameter(this.mqvParameters.getOtherPartyEphemeralKey()));
            }
        } else if (obj instanceof ECDHCUnifiedAgreement) {
            cipherParametersGeneratePublicKeyParameter = new ECDHUPublicParameters((ECPublicKeyParameters) ECUtils.generatePublicKeyParameter((PublicKey) key), (ECPublicKeyParameters) ECUtils.generatePublicKeyParameter(this.dheParameters.getOtherPartyEphemeralKey()));
        } else {
        ...
        try {
            ...
            throw new InvalidKeyException(AbstractC0030c.m117h(e2, new StringBuilder("calculation failed: "))) { // from class: org.bouncycastle.jcajce.provider.asymmetric.ec.KeyAgreementSpi.1
