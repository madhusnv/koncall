...
public class NTRUKeyPairGenerator implements AsymmetricCipherKeyPairGenerator {
    ...
    public AsymmetricCipherKeyPair generateKeyPair() {
        ...
        OWCPAKeyPair oWCPAKeyPairKeypair = new NTRUOWCPA(nTRUParameterSet).keypair(bArr);
        byte[] bArr2 = oWCPAKeyPairKeypair.publicKey;
        byte[] bArr3 = new byte[nTRUParameterSet.ntruSecretKeyBytes()];
        byte[] bArr4 = oWCPAKeyPairKeypair.privateKey;
        System.arraycopy(bArr4, 0, bArr3, 0, bArr4.length);
        int iPrfKeyBytes = nTRUParameterSet.prfKeyBytes();
        byte[] bArr5 = new byte[iPrfKeyBytes];
        this.random.nextBytes(bArr5);
        System.arraycopy(bArr5, 0, bArr3, nTRUParameterSet.owcpaSecretKeyBytes(), iPrfKeyBytes);
