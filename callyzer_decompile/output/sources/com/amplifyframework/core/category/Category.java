...
public abstract class Category<P extends Plugin<?>> implements CategoryTypeable {
    ...
    private P getPluginIfConfiguredOrThrow(P p4) {
        if (p4 == null) {
        ...
        if (State.INITIALIZATION_FAILED.equals(this.state.get())) {
            ...
            throw new IllegalStateException("Failed to get plugin because initialization previously failed.  See attached exception for details.", categoryInitializationResult != null ? categoryInitializationResult.getPluginInitializationFailures().get(p4.getPluginKey()) : null);
        }
        if (isConfigured()) {
            return p4;
        }
    ...
    public final void addPlugin(P p4) throws AmplifyException {
        if (State.NOT_CONFIGURED.equals(this.state.get())) {
            this.plugins.put(p4.getPluginKey(), p4);
        } else {
            ...
            try {
                for (P p4 : getPlugins()) {
                    if (configureFromDefaultConfigFile()) {
                        JSONObject pluginConfig = categoryConfiguration.getPluginConfig(p4.getPluginKey());
                        if (pluginConfig == null) {
                            pluginConfig = new JSONObject();
                        }
                        p4.configure(pluginConfig, context);
                    }
                }
    ...
    public synchronized CategoryInitializationResult initialize(Context context) {
        ...
        try {
            ...
            if (State.CONFIGURED.equals(this.state.get())) {
                this.state.set(State.INITIALIZING);
                for (P p4 : getPlugins()) {
                    try {
                        p4.initialize(context);
                        initializationResultFailure = InitializationResult.success();
                    } catch (AmplifyException e2) {
                        initializationResultFailure = InitializationResult.failure(e2);
                    }
                    map.put(p4.getPluginKey(), initializationResultFailure);
                }
            } else {
                Iterator<P> it = getPlugins().iterator();
                while (it.hasNext()) {
                    map.put(it.next().getPluginKey(), InitializationResult.failure(new AmplifyException("Tried to init before category was configured.", "Call configure() on category, first.")));
                }
            }
    ...
    public final void removePlugin(P p4) {
        this.plugins.remove(p4.getPluginKey());
    }
        ...
        synchronized (this.state) {
            if (State.NOT_CONFIGURED.equals(this.state.get())) {
                ...
                try {
                    for (P p4 : getPlugins()) {
                        if (configureFromDefaultConfigFile()) {
                            p4.configure(amplifyOutputsData, context);
                        }
                    }
                    this.state.set(State.CONFIGURED);
                } catch (Throwable th2) {
