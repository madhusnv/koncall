...
public final class Amplify {
    ...
    public static final class AlreadyConfiguredException extends AmplifyException {
        ...
        public /* synthetic */ AlreadyConfiguredException(String str, int i10) {
            this(str);
        }
    ...
    public static <P extends Plugin<?>> void addPlugin(P p4) {
        updatePluginRegistry(p4, RegistryUpdateType.ADD);
    }

    @AmplifyFlutterApi
    public static void addUserAgentPlatform(UserAgent.Platform platform, String str) {
        Map.Entry[] entryArr = {new AbstractMap.SimpleEntry(platform, str)};
        ...
        if (map.put(key, value) != null) {
            throw new IllegalArgumentException(AbstractC0030c.m118i(key, "duplicate key: "));
        }
        UserAgent.configure(Collections.unmodifiableMap(map));
    }

    private static void beginInitialization(Category<? extends Plugin<?>> category, Context context) {
        INITIALIZATION_POOL.execute(new RunnableC0012k(8, category, context));
    }
    ...
    public static <P extends Plugin<?>> void removePlugin(P p4) {
        updatePluginRegistry(p4, RegistryUpdateType.REMOVE);
    }

    private static <P extends Plugin<?>> void updatePluginRegistry(P p4, RegistryUpdateType registryUpdateType) {
        Category<? extends Plugin<?>> category;
        AtomicBoolean atomicBoolean = CONFIGURATION_LOCK;
        synchronized (atomicBoolean) {
            if (atomicBoolean.get()) {
                throw new AlreadyConfiguredException("Do not add plugins after calling `Amplify.configure()`.", 0);
            }
            if (Empty.check(p4.getPluginKey())) {
                throw new AmplifyException("Plugin key was missing for + ".concat(p4.getClass().getSimpleName()), "This should never happen - contact the plugin developers to find out why this is.");
            }
            LinkedHashMap<CategoryType, Category<? extends Plugin<?>>> linkedHashMap = CATEGORIES;
            if (!linkedHashMap.containsKey(p4.getCategoryType())) {
            ...
            try {
                category = linkedHashMap.get(p4.getCategoryType());
            ...
            if (RegistryUpdateType.REMOVE.equals(registryUpdateType)) {
                category.removePlugin(p4);
            } else {
                category.addPlugin(p4);
            }
        }
    }
    ...
    public static void configure(AmplifyConfiguration amplifyConfiguration, Context context) {
        ...
        synchronized (atomicBoolean) {
            try {
                if (atomicBoolean.get()) {
                    throw new AlreadyConfiguredException("Remove the duplicate call to `Amplify.configure()`.", 0);
                }
