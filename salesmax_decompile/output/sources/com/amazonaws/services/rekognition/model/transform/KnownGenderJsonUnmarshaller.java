package com.amazonaws.services.rekognition.model.transform;

import com.amazonaws.services.rekognition.model.KnownGender;
import com.amazonaws.transform.JsonUnmarshallerContext;
import com.amazonaws.transform.SimpleTypeJsonUnmarshallers;
import com.amazonaws.transform.Unmarshaller;
import com.amazonaws.util.json.AwsJsonReader;

/* loaded from: classes5.dex */
class KnownGenderJsonUnmarshaller implements Unmarshaller<KnownGender, JsonUnmarshallerContext> {
    private static KnownGenderJsonUnmarshaller instance;

    public static KnownGenderJsonUnmarshaller getInstance() {
        if (instance == null) {
            instance = new KnownGenderJsonUnmarshaller();
        }
        return instance;
    }

    @Override // com.amazonaws.transform.Unmarshaller
    public KnownGender unmarshall(JsonUnmarshallerContext jsonUnmarshallerContext) {
        AwsJsonReader reader = jsonUnmarshallerContext.getReader();
        if (!reader.isContainer()) {
            reader.skipValue();
            return null;
        }
        KnownGender knownGender = new KnownGender();
        reader.beginObject();
        while (reader.hasNext()) {
            if (reader.nextName().equals("Type")) {
                knownGender.setType(SimpleTypeJsonUnmarshallers.StringJsonUnmarshaller.getInstance().unmarshall(jsonUnmarshallerContext));
            } else {
                reader.skipValue();
            }
        }
        reader.endObject();
        return knownGender;
    }
}
