...
class DefaultSettingsSpiCall implements SettingsSpiCall {
    ...
    private JSONObject getJsonObjectFrom(String str) {
        try {
            ...
            this.logger.m15671w("Failed to parse settings JSON from " + this.url, e);
            this.logger.m15670w("Settings response " + str);
            return null;
        }
    }
    ...
    public JSONObject handleResponse(HttpResponse httpResponse) {
        ...
        this.logger.m15668v("Settings response code was: " + iCode);
        if (requestWasSuccessful(iCode)) {
            return getJsonObjectFrom(httpResponse.body());
        }
        this.logger.m15664e("Settings request failed; (status: " + iCode + ") from " + this.url);
        return null;
    }
    ...
    public JSONObject invoke(SettingsRequest settingsRequest, boolean z) {
        ...
        try {
            ...
            this.logger.m15662d("Requesting settings from " + this.url);
            this.logger.m15668v("Settings query params were: " + queryParamsFor);
            return handleResponse(httpGetRequestApplyHeadersTo.execute());
        } catch (IOException e) {
            this.logger.m15665e("Settings request failed.", e);
            return null;
        }
    }
