...
public class SettingsController implements SettingsProvider {
                ...
                if (cachedSettings != null) {
                    ...
                    if (settingsJson != null) {
                        ...
                        if (SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(settingsCacheBehavior) || !settingsJson.isExpired(currentTimeMillis)) {
                            try {
                                Logger.getLogger().m15668v("Returning cached settings.");
                                settings = settingsJson;
                            } catch (Exception e) {
                                e = e;
                                settings = settingsJson;
                                Logger.getLogger().m15665e("Failed to get cached settings", e);
                                return settings;
                            }
                        } else {
                            Logger.getLogger().m15668v("Cached settings have expired.");
                        }
                    } else {
                        Logger.getLogger().m15665e("Failed to parse cached settings data.", null);
                    }
                } else {
                    Logger.getLogger().m15662d("No cached settings data found.");
                }
            }
        } catch (Exception e2) {
            e = e2;
    ...
    public void logSettings(JSONObject jSONObject, String str) {
        Logger.getLogger().m15662d(str + jSONObject.toString());
    }
    ...
    public Task<Void> loadSettingsData(SettingsCacheBehavior settingsCacheBehavior, Executor executor) throws Throwable {
        ...
        return this.dataCollectionArbiter.waitForDataCollectionPermission(executor).onSuccessTask(executor, new SuccessContinuation<Void, Void>() { // from class: com.google.firebase.crashlytics.internal.settings.SettingsController.1
            ...
            public Task<Void> then(Void r5) throws Throwable {
