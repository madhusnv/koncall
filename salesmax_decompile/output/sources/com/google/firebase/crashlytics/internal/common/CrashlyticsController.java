...
class CrashlyticsController {
    ...
    public class C113304 implements SuccessContinuation<Boolean, Void> {
        final /* synthetic */ Task val$settingsDataTask;

        public C113304(Task task) {
            ...
            return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1
                ...
                public Task<Void> call() {
                    if (bool.booleanValue()) {
                        Logger.getLogger().m15662d("Sending cached crash reports...");
                        CrashlyticsController.this.dataCollectionArbiter.grantDataCollectionPermission(bool.booleanValue());
                        final Executor executor = CrashlyticsController.this.backgroundWorker.getExecutor();
                        return C113304.this.val$settingsDataTask.onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.4.1.1
                            @Override // com.google.android.gms.tasks.SuccessContinuation
                            public Task<Void> then(Settings settings) {
                                if (settings == null) {
                                    Logger.getLogger().m15670w("Received null app settings at app startup. Cannot send cached reports");
                    ...
                    Logger.getLogger().m15668v("Deleting cached crash reports...");
    ...
    public void doOpenSession(String str) {
        ...
        Logger.getLogger().m15662d("Opening a new session with ID " + str);
    ...
    public void doWriteAppExceptionMarker(long j) throws IOException {
        try {
            ...
            Logger.getLogger().m15671w("Could not create app exception marker file.", e);
        }
    }

    private void finalizePreviousNativeSession(String str) throws IOException {
        Logger.getLogger().m15668v("Finalizing native report for session " + str);
        NativeSessionFileProvider sessionFileProvider = this.nativeComponent.getSessionFileProvider(str);
        File minidumpFile = sessionFileProvider.getMinidumpFile();
        if (minidumpFile == null || !minidumpFile.exists()) {
            Logger.getLogger().m15670w("No minidump data found for session " + str);
            return;
        }
        ...
        if (!nativeSessionDir.isDirectory()) {
            Logger.getLogger().m15670w("Couldn't create directory to store native session files, aborting.");
            return;
        }
        ...
        Logger.getLogger().m15662d("CrashlyticsController#finalizePreviousNativeSession");
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().m15670w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().m15662d("Logging app exception event to Firebase Analytics");
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().m15670w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().m15662d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(Boolean.TRUE);
        }
        Logger.getLogger().m15662d("Automatic data collection is disabled.");
        Logger.getLogger().m15668v("Notifying that unsent reports are available.");
        ...
        Logger.getLogger().m15662d("Waiting for send/deleteUnsentReports to be called.");
    ...
    private void writeApplicationExitInfoEventIfRelevant(String str) {
        ...
        if (i < 30) {
            Logger.getLogger().m15668v("ANR feature enabled, but device is API " + i);
            return;
        }
        ...
        if (historicalProcessExitReasons.size() != 0) {
            ...
            Logger.getLogger().m15668v("No ApplicationExitInfo available. Session: " + str);
        }
    }
    ...
    public Task<Boolean> checkForUnsentReports() {
        ...
        Logger.getLogger().m15670w("checkForUnsentReports should only be called once per execution.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().m15668v("Found previous crash marker.");
    ...
    public boolean finalizeSessions(SettingsProvider settingsProvider) {
        ...
        if (isHandlingException()) {
            Logger.getLogger().m15670w("Skipping session finalization because a crash has already occurred.");
            return false;
        }
        Logger.getLogger().m15668v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true, settingsProvider);
            Logger.getLogger().m15668v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().m15665e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void logFatalException(Thread thread, Throwable th) {
        ...
        if (settingsProvider == null) {
            Logger.getLogger().m15670w("settingsProvider not set");
        } else {
    ...
    public void setCustomKey(String str, String str2) {
        try {
            ...
            Logger.getLogger().m15664e("Attempting to set custom attribute with null key, ignoring.");
        }
    }
    ...
    public Task<Void> submitAllReports(Task<Settings> task) {
        if (this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().m15668v("Crash reports are available to be sent.");
            return waitForReportAction().onSuccessTask(new C113304(task));
        }
        Logger.getLogger().m15668v("No crash reports are available to be sent.");
        this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
        return Tasks.forResult(null);
    }
        ...
        this.backgroundWorker.submit(new Runnable() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.6
            ...
            public void run() {
                ...
                if (currentSessionId == null) {
                    Logger.getLogger().m15670w("Tried to write a non-fatal exception while no session was open.");
                } else {
    ...
    private void doCloseSessions(boolean z, SettingsProvider settingsProvider) throws IOException {
        ...
        if (arrayList.size() <= z) {
            Logger.getLogger().m15668v("No open sessions to be closed.");
            return;
        }
        ...
        if (settingsProvider.getSettingsSync().featureFlagData.collectAnrs) {
            ...
            Logger.getLogger().m15668v("ANR feature disabled.");
        }
    ...
    public synchronized void handleUncaughtException(final SettingsProvider settingsProvider, final Thread thread, final Throwable th, final boolean z) {
        Logger.getLogger().m15662d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        final long jCurrentTimeMillis = System.currentTimeMillis();
        try {
            Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2
                ...
                public Task<Void> call() throws IOException {
                    ...
                    if (currentSessionId == null) {
                        Logger.getLogger().m15664e("Tried to write a fatal exception while no session was open.");
                        return Tasks.forResult(null);
                    }
                    ...
                    return settingsProvider.getSettingsAsync().onSuccessTask(executor, new SuccessContinuation<Settings, Void>() { // from class: com.google.firebase.crashlytics.internal.common.CrashlyticsController.2.1
                        ...
                        public Task<Void> then(Settings settings) {
                            if (settings == null) {
                                Logger.getLogger().m15670w("Received null app settings, cannot send reports at crash time.");
                                return Tasks.forResult(null);
                            }
            ...
            Logger.getLogger().m15664e("Cannot send reports. Timed out while fetching settings.");
        } catch (Exception e) {
            Logger.getLogger().m15665e("Error handling uncaught exception", e);
        }
    }
}
