...
class CrashlyticsUncaughtExceptionHandler implements Thread.UncaughtExceptionHandler {
    ...
    private boolean shouldRecordUncaughtException(Thread thread, Throwable th) {
        if (thread == null) {
            Logger.getLogger().m15664e("Crashlytics will not record uncaught exception; null thread");
            return false;
        }
        if (th == null) {
            Logger.getLogger().m15664e("Crashlytics will not record uncaught exception; null throwable");
            return false;
        }
        if (!this.nativeComponent.hasCrashDataForCurrentSession()) {
            return true;
        }
        Logger.getLogger().m15662d("Crashlytics will not record uncaught exception; native crash exists for session.");
        return false;
    }
        ...
        try {
            try {
                if (shouldRecordUncaughtException(thread, th)) {
                    ...
                    Logger.getLogger().m15662d("Uncaught exception will not be recorded by Crashlytics.");
                }
            } catch (Exception e) {
                Logger.getLogger().m15665e("An error occurred in the uncaught exception handler", e);
            }
        } finally {
            Logger.getLogger().m15662d(r0);
