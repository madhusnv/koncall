...
public abstract class zzpy<V> extends zzqx implements zfa {
    ...
    /* JADX WARN: Multi-variable type inference failed */
    private static Object zzn(zfa zfaVar) {
        Throwable thZzf;
        if (zfaVar instanceof zzh) {
            Object zzbVar = ((zzpy) zfaVar).value;
        ...
        if ((zfaVar instanceof zzqx) && (thZzf = ((zzqx) zfaVar).zzf()) != null) {
        ...
        try {
            Object objZzo = zzo(zfaVar);
            if (!zIsCancelled) {
                return objZzo == null ? zzd : objZzo;
            }
            return new zzb(false, new IllegalArgumentException("get() did not throw CancellationException, despite reporting isCancelled() == true: " + String.valueOf(zfaVar)));
        } catch (Error e) {
            e = e;
            return new zzc(e);
        } catch (CancellationException e2) {
            return !zIsCancelled ? new zzc(new IllegalArgumentException("get() threw CancellationException, despite reporting isCancelled() == false: ".concat(String.valueOf(zfaVar)), e2)) : new zzb(false, e2);
        } catch (RuntimeException e3) {
            e = e3;
            return new zzc(e);
        } catch (ExecutionException e4) {
            return zIsCancelled ? new zzb(false, new IllegalArgumentException("get() did not throw CancellationException, despite reporting isCancelled() == true: ".concat(String.valueOf(zfaVar)), e4)) : new zzc(e4.getCause());
        }
    }
