package com.amplifyframework.predictions.aws.models.liveness;

import java.util.List;
import p001o.dc9;
import p001o.h76;
import p001o.i4i;
import p001o.jo3;
import p001o.lo3;
import p001o.sq8;
import p001o.ubd;
import p001o.uef;
import p001o.w75;
import p001o.yn7;

/* loaded from: classes5.dex */
public final class FreshnessColor$$serializer implements yn7 {
    public static final FreshnessColor$$serializer INSTANCE;
    private static final /* synthetic */ ubd descriptor;

    static {
        FreshnessColor$$serializer freshnessColor$$serializer = new FreshnessColor$$serializer();
        INSTANCE = freshnessColor$$serializer;
        ubd ubdVar = new ubd("com.amplifyframework.predictions.aws.models.liveness.FreshnessColor", freshnessColor$$serializer, 1);
        ubdVar.m51338k("RGB", false);
        descriptor = ubdVar;
    }

    private FreshnessColor$$serializer() {
    }

    @Override // p001o.yn7
    public dc9[] childSerializers() {
        return new dc9[]{FreshnessColor.$childSerializers[0]};
    }

    @Override // p001o.bn5
    public FreshnessColor deserialize(w75 w75Var) {
        List list;
        sq8.m48649h(w75Var, "decoder");
        uef descriptor2 = getDescriptor();
        jo3 jo3VarMo20420b = w75Var.mo20420b(descriptor2);
        dc9[] dc9VarArr = FreshnessColor.$childSerializers;
        int i = 1;
        if (jo3VarMo20420b.mo17245n()) {
            list = (List) jo3VarMo20420b.mo17252u(descriptor2, 0, dc9VarArr[0], null);
        } else {
            int i2 = 0;
            List list2 = null;
            while (i != 0) {
                int iMo16814k = jo3VarMo20420b.mo16814k(descriptor2);
                if (iMo16814k == -1) {
                    i = 0;
                } else {
                    if (iMo16814k != 0) {
                        throw new i4i(iMo16814k);
                    }
                    list2 = (List) jo3VarMo20420b.mo17252u(descriptor2, 0, dc9VarArr[0], list2);
                    i2 |= 1;
                }
            }
            list = list2;
            i = i2;
        }
        jo3VarMo20420b.mo18548c(descriptor2);
        return new FreshnessColor(i, list, null);
    }

    @Override // p001o.dc9, p001o.iff, p001o.bn5
    public uef getDescriptor() {
        return descriptor;
    }

    @Override // p001o.iff
    public void serialize(h76 h76Var, FreshnessColor freshnessColor) {
        sq8.m48649h(h76Var, "encoder");
        sq8.m48649h(freshnessColor, "value");
        uef descriptor2 = getDescriptor();
        lo3 lo3VarMo29876b = h76Var.mo29876b(descriptor2);
        lo3VarMo29876b.mo37566l(descriptor2, 0, FreshnessColor.$childSerializers[0], freshnessColor.rGB);
        lo3VarMo29876b.mo37560c(descriptor2);
    }

    @Override // p001o.yn7
    public dc9[] typeParametersSerializers() {
        return yn7.C18436a.m58011a(this);
    }
}
