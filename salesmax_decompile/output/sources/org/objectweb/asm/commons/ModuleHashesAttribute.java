...
/* loaded from: salesmax.apk:org/objectweb/asm/commons/ModuleHashesAttribute.SCL.lombok */
    ...
    protected Attribute read(ClassReader classReader, int offset, int length, char[] charBuffer, int codeAttributeOffset, Label[] labels) {
        ...
        for (int i = 0; i < numModules; i++) {
            ...
            for (int j = 0; j < hashLength; j++) {
                hash[j] = (byte) (classReader.readByte(currentOffset2) & Constants.MAX_HOST_LENGTH);
                currentOffset2++;
            }
            hashList.add(hash);
        }
