...
/* loaded from: salesmax.apk:org/objectweb/asm/Constants.SCL.lombok */
final class Constants {
    ...
    static boolean isWhitelisted(String internalName) {
        if (internalName.startsWith("org/objectweb/asm/")) {
            return internalName.contains("Test$") || Pattern.matches(new StringBuilder().append("org/objectweb/asm/util/Trace").append("(Annotation|Class|Field|Method|Module|RecordComponent|Signature)").append("Visitor(\\$.*)?").toString(), internalName) || Pattern.matches(new StringBuilder().append("org/objectweb/asm/util/Check").append("(Annotation|Class|Field|Method|Module|RecordComponent|Signature)").append("Adapter(\\$.*)?").toString(), internalName);
        }
        return false;
    }
