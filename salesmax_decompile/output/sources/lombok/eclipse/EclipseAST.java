...
/* loaded from: salesmax.apk:lombok/eclipse/EclipseAST.SCL.lombok */
public class EclipseAST extends AST<EclipseAST, EclipseNode, ASTNode> {
    ...
    /* loaded from: salesmax.apk:lombok/eclipse/EclipseAST$EclipseWorkspaceBasedFileResolver.SCL.lombok */
    ...
    /* loaded from: salesmax.apk:lombok/eclipse/EclipseAST$ParseProblem.SCL.lombok */
    class ParseProblem {
    ...
    private List<EclipseNode> buildTypes(TypeDeclaration[] children) {
        ...
        if (children != null) {
            for (TypeDeclaration type2 : children) {
                addIfNotNull(childNodes, buildType(type2));
            }
        }
        return childNodes;
    }

    private EclipseNode buildType(TypeDeclaration type2) {
        if (setAndGetAsHandled(type2)) {
            return null;
        }
        List<EclipseNode> childNodes = new ArrayList<>();
        childNodes.addAll(buildFields(type2.fields, EclipseHandlerUtil.getRecordFieldAnnotations(type2)));
        childNodes.addAll(buildTypes(type2.memberTypes));
        childNodes.addAll(buildMethods(type2.methods));
        childNodes.addAll(buildAnnotations(type2.annotations, false));
        return putInMap(new EclipseNode(this, type2, childNodes, AST.Kind.TYPE));
    }
    ...
    /* loaded from: salesmax.apk:lombok/eclipse/EclipseAST$EcjReflectionCheck.SCL.lombok */
