...
/* loaded from: salesmax.apk:lombok/javac/JavacASTVisitor.SCL.lombok */
public interface JavacASTVisitor {
    ...
    /* loaded from: salesmax.apk:lombok/javac/JavacASTVisitor$Printer.SCL.lombok */
    public static class Printer implements JavacASTVisitor {
        ...
        @Override // lombok.javac.JavacASTVisitor
        public void visitType(JavacNode node, JCTree.JCClassDecl type2) {
            print("<TYPE %s> %s", type2.name, printFlags(type2.mods.flags));
            this.indent++;
        ...
        @Override // lombok.javac.JavacASTVisitor
        public void visitAnnotationOnType(JCTree.JCClassDecl type2, JavacNode node, JCTree.JCAnnotation annotation) {
            forcePrint("<ANNOTATION: %s />", annotation);
        }

        @Override // lombok.javac.JavacASTVisitor
        public void endVisitType(JavacNode node, JCTree.JCClassDecl type2) {
            if (this.printContent) {
                this.disablePrinting--;
            }
            this.indent--;
            print("</TYPE %s>", type2.name);
        }
        ...
        public void visitMethod(JavacNode node, JCTree.JCMethodDecl method) throws NoSuchFieldException {
            ...
            if (method.name.contentEquals("<init>")) {
                type2 = (method.mods.flags & 68719476736L) != 0 ? "DEFAULTCONSTRUCTOR" : "CONSTRUCTOR";
            } else {
                type2 = "METHOD";
            }
            print("<%s %s> %s returns: %s", type2, method.name, printFlags(method.mods.flags), method.restype);
            this.indent++;
            try {
