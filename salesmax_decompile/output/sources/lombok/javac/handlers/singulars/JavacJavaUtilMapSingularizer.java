...
/* loaded from: salesmax.apk:lombok/javac/handlers/singulars/JavacJavaUtilMapSingularizer.SCL.lombok */
public class JavacJavaUtilMapSingularizer extends JavacJavaUtilSingularizer {
    ...
    public List<JavacNode> generateFields(JavacSingularsRecipes.SingularData data, JavacNode builderType, JavacNode source) {
        ...
        JCTree.JCVariableDecl buildKeyField = maker.VarDef(maker.Modifiers(2L), builderType.toName(data.getPluralName() + "$key"), addTypeArgs(1, false, builderType, type2, data.getTypeArgs(), source), null);
        JCTree.JCExpression type3 = JavacHandlerUtil.chainDots(builderType, "java", "util", "ArrayList");
        com.sun.tools.javac.util.List<JCTree.JCExpression> tArgs = data.getTypeArgs();
        JCTree.JCVariableDecl buildValueField = maker.VarDef(maker.Modifiers(2L), builderType.toName(data.getPluralName() + "$value"), addTypeArgs(1, false, builderType, type3, (tArgs == null || tArgs.size() <= 1) ? com.sun.tools.javac.util.List.nil() : tArgs.tail, source), null);
