...
/* loaded from: salesmax.apk:lombok/javac/handlers/HandleConstructor.SCL.lombok */
public class HandleConstructor {
    private static /* synthetic */ int[] $SWITCH_TABLE$javax$lang$model$type$TypeKind;

    /* loaded from: salesmax.apk:lombok/javac/handlers/HandleConstructor$SkipIfConstructorExists.SCL.lombok */
    ...
    /* loaded from: salesmax.apk:lombok/javac/handlers/HandleConstructor$HandleNoArgsConstructor.SCL.lombok */
    ...
    /* loaded from: salesmax.apk:lombok/javac/handlers/HandleConstructor$HandleRequiredArgsConstructor.SCL.lombok */
    ...
    /* loaded from: salesmax.apk:lombok/javac/handlers/HandleConstructor$HandleAllArgsConstructor.SCL.lombok */
    ...
    private void generateStaticConstructor(boolean staticConstrRequired, JavacNode typeNode, String staticName, AccessLevel level, boolean allToDefault, List<JavacNode> fields, JavacNode source) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        if (staticConstrRequired) {
    ...
    public static JCTree.JCMethodDecl createConstructor(AccessLevel level, List<JCTree.JCAnnotation> onConstructor, JavacNode typeNode, List<JavacNode> fieldsToParam, boolean forceDefaults, JavacNode source) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        boolean addConstructorProperties;
        JCTree.JCStatement nullCheck;
        JavacTreeMaker maker = typeNode.getTreeMaker();
        boolean isEnum = (typeNode.get().mods.flags & MediaStatus.COMMAND_LIKE) != 0;
        if (isEnum) {
    ...
    private static JCTree.JCExpression getDefaultExpr(JavacTreeMaker maker, JCTree.JCExpression type2) {
        if (type2 instanceof JCTree.JCPrimitiveTypeTree) {
            switch ($SWITCH_TABLE$javax$lang$model$type$TypeKind()[((JCTree.JCPrimitiveTypeTree) type2).getPrimitiveTypeKind().ordinal()]) {
                case 1:
                ...
                case 8:
                    return maker.Literal(Javac.CTC_DOUBLE, Double.valueOf(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE));
            }
        }
    ...
    public JCTree.JCMethodDecl createStaticConstructor(String name, AccessLevel level, JavacNode typeNode, List<JavacNode> fields, JavacNode source) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        JavacTreeMaker maker = typeNode.getTreeMaker();
        ...
        if (!type2.typarams.isEmpty()) {
            Iterator it = type2.typarams.iterator();
            while (it.hasNext()) {
        ...
        JCTree.JCExpression returnType = JavacHandlerUtil.namePlusTypeParamsToTypeReference(maker, typeNode, type2.typarams, annsOnReturnType);
        JCTree.JCExpression constructorType = JavacHandlerUtil.namePlusTypeParamsToTypeReference(maker, typeNode, type2.typarams);
