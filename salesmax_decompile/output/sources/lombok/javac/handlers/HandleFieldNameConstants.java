...
/* loaded from: salesmax.apk:lombok/javac/handlers/HandleFieldNameConstants.SCL.lombok */
public class HandleFieldNameConstants extends JavacAnnotationHandler<FieldNameConstants> {
    ...
    private void createInnerTypeFieldNameConstants(JavacNode typeNode, JavacNode errorNode, AccessLevel level, List<JavacNode> fields, boolean asEnum, IdentifierName innerTypeName, boolean uppercase) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        boolean genConstr;
        ...
        JCTree.JCModifiers mods = maker.Modifiers(JavacHandlerUtil.toJavacModifier(level) | (asEnum ? Opcodes.ACC_ENUM : 24));
        ...
        if (fieldsType == null) {
            ...
            if (asEnum && (f & MediaStatus.COMMAND_LIKE) == 0) {
        ...
        for (JavacNode field : fields) {
            ...
            if (JavacHandlerUtil.fieldExists(fName.toString(), fieldsType) == JavacHandlerUtil.MemberExistsResult.NOT_EXISTS) {
                JCTree.JCModifiers constantValueMods = maker.Modifiers(25 | (asEnum ? MediaStatus.COMMAND_LIKE : 0L));
                if (asEnum) {
