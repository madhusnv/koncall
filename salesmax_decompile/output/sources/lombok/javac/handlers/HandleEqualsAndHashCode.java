...
/* loaded from: salesmax.apk:lombok/javac/handlers/HandleEqualsAndHashCode.SCL.lombok */
public class HandleEqualsAndHashCode extends JavacAnnotationHandler<EqualsAndHashCode> {
    ...
    private static /* synthetic */ int[] f13966x2486df07;
    ...
    /* renamed from: $SWITCH_TABLE$lombok$javac$handlers$JavacHandlerUtil$MemberExistsResult */
    static /* synthetic */ int[] m16265x2486df07() {
        int[] iArr = f13966x2486df07;
        if (iArr != null) {
            return iArr;
        }
    ...
    public void generateMethods(JavacNode typeNode, JavacNode source, List<InclusionExclusionUtils.Included<JavacNode, EqualsAndHashCode.Include>> members, Boolean callSuper, boolean whineIfExists, boolean cacheHashCode, HandlerUtil.FieldAccess fieldAccess, com.sun.tools.javac.util.List<JCTree.JCAnnotation> onParam) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
        ...
        switch (m16265x2486df07()[((JavacHandlerUtil.MemberExistsResult) Collections.max(Arrays.asList(equalsExists, hashCodeExists))).ordinal()]) {
            case 1:
            default:
    ...
    private void createHashCodeCacheField(JavacNode typeNode, JavacNode source) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    public JCTree.JCExpression createTypeReference(JavacNode type2, boolean addWildcards) {
        List<String> list = new ArrayList<>();
        List<Integer> genericsCount = addWildcards ? new ArrayList<>() : null;
        list.add(type2.getName());
        if (addWildcards) {
            genericsCount.add(Integer.valueOf(type2.get().typarams.size()));
        }
        boolean staticContext = (type2.get().getModifiers().flags & 8) != 0;
        JavacNode javacNodeUp = type2.up();
        while (true) {
        ...
        JCTree.JCExpression chain = maker.Ident(type2.toName(list.get(0)));
        if (addWildcards) {
            chain = wildcardify(maker, chain, genericsCount.get(0).intValue());
        }
        for (int i = 1; i < list.size(); i++) {
            chain = maker.Select(chain, type2.toName(list.get(i)));
            if (addWildcards) {
    ...
    public JCTree.JCMethodDecl createEquals(JavacNode typeNode, List<InclusionExclusionUtils.Included<JavacNode, EqualsAndHashCode.Include>> members, boolean callSuper, HandlerUtil.FieldAccess fieldAccess, boolean needsCanEqual, JavacNode source, com.sun.tools.javac.util.List<JCTree.JCAnnotation> onParam) throws IllegalAccessException, IllegalArgumentException {
        JCTree.JCExpression objectType;
        JavacTreeMaker maker = typeNode.getTreeMaker();
        Name oName = typeNode.toName(NotificationBundleProcessor.PUSH_MINIFIED_BUTTONS_LIST);
    ...
    public JCTree.JCMethodDecl createCanEqual(JavacNode typeNode, JavacNode source, com.sun.tools.javac.util.List<JCTree.JCAnnotation> onParam) throws IllegalAccessException, IllegalArgumentException {
        JavacTreeMaker maker = typeNode.getTreeMaker();
    ...
    private JCTree.JCExpression unnotate(JCTree.JCExpression type2) {
        if (isJcAnnotatedType(type2) && this.jcAnnotatedTypeUnderlyingTypeField != null) {
            try {
                return (JCTree.JCExpression) this.jcAnnotatedTypeUnderlyingTypeField.get(type2);
            } catch (Exception unused) {
                return type2;
            }
        }
    ...
    private boolean isJcAnnotatedType(JCTree.JCExpression o2) {
        if (o2 == null) {
            return false;
        }
        if (!this.jcAnnotatedTypeInit) {
            try {
                this.jcAnnotatedTypeClass = Class.forName("com.sun.tools.javac.tree.JCTree$JCAnnotatedType", false, o2.getClass().getClassLoader());
