...
/* loaded from: salesmax.apk:lombok/javac/JavacNode.SCL.lombok */
public class JavacNode extends LombokNode<JavacAST, JavacNode, JCTree> {
    ...
    public boolean hasAnnotation(Class<? extends Annotation> type2) {
        return JavacHandlerUtil.hasAnnotationAndDeleteIfNeccessary(type2, this);
    }

    @Override // lombok.core.LombokNode
    public <Z extends Annotation> AnnotationValues<Z> findAnnotation(Class<Z> type2) {
        JavacNode annotation = JavacHandlerUtil.findAnnotation(type2, this, true);
        if (annotation == null) {
            return null;
        }
        return JavacHandlerUtil.createAnnotation(type2, annotation);
    }
    ...
    public boolean isStatic() {
        ...
        if (this.node instanceof JCTree.JCClassDecl) {
            ...
            long f = p.mods.flags;
            if ((512 & f) != 0 || (MediaStatus.COMMAND_LIKE & f) != 0) {
                return true;
            }
        }
    ...
    public boolean isEnumMember() {
        ...
        return (getKind() != AST.Kind.FIELD || (mods = getModifiers()) == null || (MediaStatus.COMMAND_LIKE & mods.flags) == 0) ? false : true;
    }

    @Override // lombok.core.LombokNode
    public boolean isEnumType() {
        JCTree.JCModifiers mods;
        return (getKind() != AST.Kind.TYPE || (mods = getModifiers()) == null || (MediaStatus.COMMAND_LIKE & mods.flags) == 0) ? false : true;
    }
