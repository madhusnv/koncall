...
/* loaded from: salesmax.apk:lombok/core/handlers/InclusionExclusionUtils.SCL.lombok */
public class InclusionExclusionUtils {
    private static List<Integer> createListOfNonExistentFields(List<String> list, LombokNode<?, ?, ?> type2, boolean excludeStandard, boolean excludeTransient) {
        boolean[] matched = new boolean[list.size()];
        Iterator it = type2.down().iterator();
        while (it.hasNext()) {
    ...
    public static void checkForBogusFieldNames(LombokNode<?, ?, ?> type2, AnnotationValues<?> annotation, List<String> excludes, List<String> includes) {
        if (excludes != null && !excludes.isEmpty()) {
            Iterator<Integer> it = createListOfNonExistentFields(excludes, type2, true, false).iterator();
            while (it.hasNext()) {
        ...
        if (includes != null && !includes.isEmpty()) {
            Iterator<Integer> it2 = createListOfNonExistentFields(includes, type2, false, false).iterator();
            while (it2.hasNext()) {
    ...
    /* loaded from: salesmax.apk:lombok/core/handlers/InclusionExclusionUtils$Included.SCL.lombok */
    ...
    private static String innerAnnName(Class<? extends Annotation> type2) {
        String name = type2.getSimpleName();
        Class<?> enclosingClass = type2.getEnclosingClass();
        while (true) {
