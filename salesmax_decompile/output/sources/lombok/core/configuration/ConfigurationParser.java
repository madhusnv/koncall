...
/* loaded from: salesmax.apk:lombok/core/configuration/ConfigurationParser.SCL.lombok */
public class ConfigurationParser {
    ...
    /* loaded from: salesmax.apk:lombok/core/configuration/ConfigurationParser$Collector.SCL.lombok */
                ...
                if (importMatcher.matches()) {
                    ...
                    if (!matcher.matches()) {
                        ...
                        if (key == null) {
                            ...
                            ConfigurationDataType type2 = key.getType();
                            boolean listOperator = operator.equals("+=") || operator.equals("-=");
                            if (listOperator && !type2.isList()) {
                            ...
                            } else if (operator.equals("=") && type2.isList()) {
                                ...
                                if (stringValue != null) {
                                    try {
                                        value = type2.getParser().parse(stringValue);
                                    } catch (Exception unused) {
                                        this.reporter.report(context.description(), "Error while parsing the value for '" + keyName + "' value '" + stringValue + "' (should be " + type2.getParser().exampleValue() + ")", lineNumber, line);
                                    }
                                }
