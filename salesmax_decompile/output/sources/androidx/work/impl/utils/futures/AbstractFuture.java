...
public abstract class AbstractFuture<V> implements zfa {
    ...
    public static final class SafeAtomicHelper extends AtomicHelper {
        ...
        public boolean casListeners(AbstractFuture<?> abstractFuture, Listener listener, Listener listener2) {
            return j9.m33417a(this.listenersUpdater, abstractFuture, listener, listener2);
        }

        @Override // androidx.work.impl.utils.futures.AbstractFuture.AtomicHelper
        public boolean casValue(AbstractFuture<?> abstractFuture, Object obj, Object obj2) {
            return j9.m33417a(this.valueUpdater, abstractFuture, obj, obj2);
        }

        @Override // androidx.work.impl.utils.futures.AbstractFuture.AtomicHelper
        public boolean casWaiters(AbstractFuture<?> abstractFuture, Waiter waiter, Waiter waiter2) {
            return j9.m33417a(this.waitersUpdater, abstractFuture, waiter, waiter2);
        }
    ...
    public static final class SetFuture<V> implements Runnable {
        ...
        public SetFuture(AbstractFuture<V> abstractFuture, zfa zfaVar) {
    ...
    public static Object getFutureValue(zfa zfaVar) {
        if (zfaVar instanceof AbstractFuture) {
            Object obj = ((AbstractFuture) zfaVar).value;
            if (!(obj instanceof Cancellation)) {
                return obj;
            }
        ...
        try {
            ...
            return new Failure(new IllegalArgumentException("get() threw CancellationException, despite reporting isCancelled() == false: " + zfaVar, e));
        } catch (ExecutionException e2) {
    ...
    public final boolean cancel(boolean z) {
        ...
        while (true) {
            if (ATOMIC_HELPER.casValue(abstractFuture, obj, cancellation)) {
                ...
                zfa zfaVar = ((SetFuture) obj).future;
                if (!(zfaVar instanceof AbstractFuture)) {
                    zfaVar.cancel(z);
                    return true;
                }
                abstractFuture = (AbstractFuture) zfaVar;
    ...
    public boolean setFuture(zfa zfaVar) {
        ...
        if (obj == null) {
            if (zfaVar.isDone()) {
                if (!ATOMIC_HELPER.casValue(this, null, getFutureValue(zfaVar))) {
                    return false;
                }
                complete(this);
                return true;
            }
            SetFuture setFuture = new SetFuture(this, zfaVar);
            if (ATOMIC_HELPER.casValue(this, null, setFuture)) {
                try {
                    zfaVar.addListener(setFuture, DirectExecutor.INSTANCE);
                } catch (Throwable th) {
                    try {
        ...
        if (obj instanceof Cancellation) {
            zfaVar.cancel(((Cancellation) obj).wasInterrupted);
        }
        return false;
    }
