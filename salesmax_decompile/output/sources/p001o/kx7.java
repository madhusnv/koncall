...
public abstract class kx7 {
    ...
    public static double[] m36277b(double d, double d2) {
        ...
        return m36288m(dArr, dArr2);
    }

    /* renamed from: c */
    public static double[][] m36278c(double d, double d2) {
        ...
        boolean z2 = true;
        for (int i = 0; i < 12; i++) {
            double[] dArrM36289n = m36289n(d, i);
            if (dArrM36289n[0] >= FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                double dM36283h = m36283h(dArrM36289n);
                if (!z) {
                    z = true;
                    dArr = dArrM36289n;
                    dArr2 = dArr;
                    d3 = dM36283h;
                    d4 = d3;
                } else if (z2 || m36276a(d3, dM36283h, d4)) {
                    if (m36276a(d3, d2, dM36283h)) {
                        z2 = false;
                        dArr2 = dArrM36289n;
                        d4 = dM36283h;
                    } else {
    ...
    /* renamed from: d */
    public static double m36279d(double d) {
        double dPow = Math.pow(Math.abs(d), 0.42d);
        return ((nza.m41320h(d) * 400.0d) * dPow) / (dPow + 27.13d);
    }

    /* renamed from: e */
    public static int m36280e(double d) {
        return (int) Math.ceil(d - 0.5d);
    }

    /* renamed from: f */
    public static int m36281f(double d) {
        return (int) Math.floor(d - 0.5d);
    }

    /* renamed from: g */
    public static int m36282g(double d, double d2, double d3) {
        double dSqrt = Math.sqrt(d3) * 11.0d;
        m0j m0jVar = m0j.f34613k;
        double dPow = 1.0d / Math.pow(1.64d - Math.pow(0.29d, m0jVar.m38149f()), 0.73d);
        double dCos = (Math.cos(d + 2.0d) + 3.8d) * 0.25d * 3846.153846153846d * m0jVar.m38151h() * m0jVar.m38152i();
        ...
        while (i < 5) {
            ...
            double d5 = dPow;
            double dPow2 = Math.pow(((d2 == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE || dSqrt == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) ? 0.0d : d2 / Math.sqrt(d4)) * dPow, 1.1111111111111112d);
            double d6 = dSqrt;
            double dM38145b = (m0jVar.m38145b() * Math.pow(d4, (1.0d / m0jVar.m38146c()) / m0jVar.m38154k())) / m0jVar.m38150g();
            double d7 = (((0.305d + dM38145b) * 23.0d) * dPow2) / (((23.0d * dCos) + ((dPow2 * 11.0d) * dCos2)) + ((dPow2 * 108.0d) * dSin));
            double d8 = d7 * dCos2;
            double d9 = d7 * dSin;
            double d10 = dM38145b * 460.0d;
            double[] dArrM41317e = nza.m41317e(new double[]{m36285j((((451.0d * d8) + d10) + (288.0d * d9)) / 1403.0d), m36285j(((d10 - (891.0d * d8)) - (261.0d * d9)) / 1403.0d), m36285j(((d10 - (d8 * 220.0d)) - (d9 * 6300.0d)) / 1403.0d)}, f32784b);
            double d11 = dArrM41317e[0];
            if (d11 < FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                break;
            }
            double d12 = dArrM41317e[1];
            if (d12 < FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                break;
            }
            double d13 = dArrM41317e[2];
            ...
            if (i != 4) {
                ...
                if (Math.abs(d15) >= 0.002d) {
                    dSqrt = d6 - ((d15 * d6) / (d14 * 2.0d));
                    i++;
                    dPow = d5;
                }
            }
            if (dArrM41317e[0] <= 100.01d && dArrM41317e[1] <= 100.01d && dArrM41317e[2] <= 100.01d) {
                return mi3.m39092b(dArrM41317e);
            }
        }
        return 0;
    }
    ...
    /* renamed from: h */
    public static double m36283h(double[] dArr) {
        double[] dArrM41317e = nza.m41317e(dArr, f32783a);
        double dM36279d = m36279d(dArrM41317e[0]);
        double dM36279d2 = m36279d(dArrM41317e[1]);
        double dM36279d3 = m36279d(dArrM41317e[2]);
        return Math.atan2(((dM36279d + dM36279d2) - (dM36279d3 * 2.0d)) / 9.0d, (((dM36279d * 11.0d) + ((-12.0d) * dM36279d2)) + dM36279d3) / 11.0d);
    }

    /* renamed from: i */
    public static double m36284i(double d, double d2, double d3) {
        return (d2 - d) / (d3 - d);
    }

    /* renamed from: j */
    public static double m36285j(double d) {
        double dAbs = Math.abs(d);
        return nza.m41320h(d) * Math.pow(Math.max(FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, (27.13d * dAbs) / (400.0d - dAbs)), 2.380952380952381d);
    }

    /* renamed from: k */
    public static boolean m36286k(double d) {
        return FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE <= d && d <= 100.0d;
    }

    /* renamed from: l */
    public static double[] m36287l(double[] dArr, double d, double[] dArr2) {
        double d2 = dArr[0];
        double d3 = dArr[1];
        double d4 = dArr[2];
    ...
    /* renamed from: m */
    public static double[] m36288m(double[] dArr, double[] dArr2) {
        return new double[]{(dArr[0] + dArr2[0]) / 2.0d, (dArr[1] + dArr2[1]) / 2.0d, (dArr[2] + dArr2[2]) / 2.0d};
    }

    /* renamed from: n */
    public static double[] m36289n(double d, int i) {
        double[] dArr = f32785c;
        double d2 = dArr[0];
        double d3 = dArr[1];
        double d4 = dArr[2];
        ...
        if (i < 4) {
            ...
            return m36286k(d7) ? new double[]{d7, d6, d5} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        if (i < 8) {
            double d8 = ((d - (d2 * d5)) - (d4 * d6)) / d3;
            return m36286k(d8) ? new double[]{d5, d8, d6} : new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d9 = ((d - (d2 * d6)) - (d3 * d5)) / d4;
        return m36286k(d9) ? new double[]{d6, d5, d9} : new double[]{-1.0d, -1.0d, -1.0d};
    }

    /* renamed from: o */
    public static double m36290o(double d) {
        return (d + 25.132741228718345d) % 6.283185307179586d;
    }

    /* renamed from: p */
    public static double[] m36291p(double[] dArr, double d, double[] dArr2, int i) {
        return m36287l(dArr, m36284i(dArr[i], d, dArr2[i]), dArr2);
    }

    /* renamed from: q */
    public static int m36292q(double d, double d2, double d3) {
        if (d2 < 1.0E-4d || d3 < 1.0E-4d || d3 > 99.9999d) {
            return mi3.m39093c(d3);
        }
        double dM41318f = (nza.m41318f(d) / 180.0d) * 3.141592653589793d;
        double dM39108r = mi3.m39108r(d3);
        int iM36282g = m36282g(dM41318f, d2, dM39108r);
        return iM36282g != 0 ? iM36282g : mi3.m39092b(m36277b(dM39108r, dM41318f));
    }

    /* renamed from: r */
    public static double m36293r(double d) {
        double d2 = d / 100.0d;
